...
import server.interfaces.ObserverInterface;
...
public class Main {
	...
	public void start(){
		...
		ServerSocket serverSocket = new ServerSocket(SERVER_PORT_TCP)){
			...
			while((clientSocket = serverSocket.accept()) != null){
				new Thread(new server.service.TcpConnection(this, clientSocket)).start();
			}
	...
	public static class IpPort implements ObserverInterface {
		...
		public void update(DatagramPacket packet) {
			...
			try {
				server.service.UdpConnection.getInstance().gamePlaySocket.send(packet);
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
